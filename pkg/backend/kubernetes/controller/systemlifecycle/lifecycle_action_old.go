...
func newLifecycleActions() *lifecycleActions {
	return &lifecycleActions{systems: make(map[types.UID]*lifecycleActionTree)}
}

type lifecycleActions struct {
	// this could be a RWMutex but trying to keep things "simple" to start
	sync.Mutex
	systems map[types.UID]*lifecycleActionTree
}

func (a *lifecycleActions) AcquireDeploy(system types.UID, deploy v1.DeployID, path tree.Path) error {
	a.Lock()
	defer a.Unlock()
...
func (a *lifecycleActions) ReleaseDeploy(system types.UID, deploy v1.DeployID) {
	a.Lock()
	defer a.Unlock()
...
func (a *lifecycleActions) AcquireTeardown(system types.UID, teardown v1.TeardownID) error {
	a.Lock()
	defer a.Unlock()
...
func (a *lifecycleActions) ReleaseTeardown(system types.UID, teardown v1.TeardownID) {
	a.Lock()
	defer a.Unlock()
