...
func (c *Controller) syncPendingRollout(rollout *latticev1.SystemRollout) error {
	...
	if currentOwningAction != nil {
		...
		_, err := c.updateRolloutStatus(rollout, latticev1.SystemRolloutStateFailed, fmt.Sprintf("another lifecycle action is active: %v", currentOwningAction.String()))
		return err
	}

	glog.V(5).Infof("SystemRollout %v/%v successfully claimed owning lifecycle action state", rollout.Namespace, rollout.Name)
	_, err := c.updateRolloutStatus(rollout, latticev1.SystemRolloutStateAccepted, "")
	return err
}
