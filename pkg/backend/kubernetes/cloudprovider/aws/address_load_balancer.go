...
// FIXME: move this to use load balancer
...
func (cp *DefaultAWSCloudProvider) ServiceAddressLoadBalancerPorts(
	...
	for _, port := range kubeService.Spec.Ports {
		...
		ports[port.Port] = fmt.Sprintf("%v", port.NodePort)
	}

	return ports, nil
}

func (cp *DefaultAWSCloudProvider) kubeServiceSpec(address *latticev1.Address, service *latticev1.Service) (corev1.ServiceSpec, error) {
