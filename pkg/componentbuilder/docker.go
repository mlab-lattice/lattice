...
func (b *Builder) buildDockerImage(sourceDirectory string) error {
	...
	if b.StatusUpdater != nil {
		...
		// not be updated.
		b.StatusUpdater.UpdateProgress(b.BuildID, b.SystemID, constants.ComponentBuildPhaseBuildingDockerImage)
	}
...
func (b *Builder) pushDockerImage() error {
	...
	if b.StatusUpdater != nil {
		// For now ignore status update errors, don't need to fail a build because the status could
		// not be updated.
		b.StatusUpdater.UpdateProgress(b.BuildID, b.SystemID, constants.ComponentBuildPhasePushingDockerImage)
	}
